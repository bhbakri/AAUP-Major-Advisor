...
const SYSTEM_LABELS = {
  ...
  ar: {
    ...
    IGCSE_GCSE_GCE: "النظام البريطاني IGCSE / GCSE / GCE",
    SAT: "شهادة SAT",
    AP: "شهادة AP",
    CLEP: "شهادة CLEP",
...
const REPORT_TEXT = {
  en: {
    ...
    yes: "Yes",
    no: "No",
    eligibleHeading: "Top Preliminarily Eligible Matches",
    reviewHeading:
    ...
    publishedCoreSubjects: "the published core subjects",
    invalidSystemTitle:
  ...
  ar: {
    title: "تقرير اختيار التخصص في الجامعة العربية الأمريكية",
    ...
    equivalencyConfirmed: "هل تم تأكيد المعدل المعادل رسمياً",
    ...
    yes: "نعم",
    no: "لا",
    eligibleHeading: "أفضل التخصصات التي يستوفي الطالب شروطها الأولية",
    reviewHeading: "تخصصات تحتاج إلى مراجعة رسمية للشهادة أو التصنيف",
    closeHeading: "تخصصات قريبة لا يستوفي الطالب أحد شروطها المنشورة حالياً",
    ...
    publishedCoreSubjects: "المباحث الأساسية المنشورة",
    invalidSystemTitle: "تعذر التعرف على نظام الشهادة",
    invalidBranchTitle: "تعذر التعرف على فرع الثانوية العامة",
    invalidAverageTitle: "تعذر قراءة معدل الطالب",
    invalidAverageRange:
      "يجب أن يكون المعدل بين 0 و100.",
    supportedSystems: "أنظمة الشهادات المدعومة",
    supportedBranches: "فروع الثانوية العامة المدعومة",
    foreignAverageHelp:
...
function branchLabel(branch, language) {
  if (!branch) return t(language).notProvided;
...
function campusMatches(majorCampus, campusPreference) {
  const preference = normalizeText(campusPreference);
  ...
  return (aliases[majorCampus] ?? [majorCampus]).some(
    (alias) => preference.includes(normalizeText(alias))
  );
}

function isAllBranchesMajor(major) {
  if (major.eligibleBranchesMode === "all") {
    return true;
  }

  return (major.eligibleBranches ?? []).some((branch) => {
...
function branchMatches(
  ...
  const eligible = new Set(
    (major.eligibleBranches ?? []).map(normalizeText)
  );

  if (certificateSystem === "Tawjihi") {
    return eligible.has(normalizeText(detectedBranch));
  }

  if (detectedBranch === "Scientific") {
    return true;
  }

  if (detectedBranch === "Literary") {
    return (
      eligible.has(normalizeText("Literary")) ||
      eligible.has(normalizeText("Sharia"))
    );
  }
...
function getTawjihiMinimum(major, detectedBranch) {
  const byBranch = major.minimumAverageTawjihiByBranch;

  if (
    byBranch &&
    detectedBranch &&
    Number.isFinite(byBranch[detectedBranch])
  ) {
    return byBranch[detectedBranch];
  }

  return Number.isFinite(major.minimumAverageTawjihi)
...
function getPublishedMinimum(
  ...
  if (certificateSystem === "Tawjihi") {
    return getTawjihiMinimum(major, detectedBranch);
  }

  if (certificateSystem === "Bagrut") {
    return Number.isFinite(major.minimumAverageBagrut)
...
function getAlternativeTawjihiRoute(
  ...
  const requirement = major.alternativeTawjihiRequirement;

  if (!requirement) return null;

  if (
    Number.isFinite(
      requirement.alternativeMinimumOverallAverage
    )
  ) {
    return requirement;
  }

  const branchRequirement = requirement[detectedBranch];

  if (
    branchRequirement &&
    Number.isFinite(
      branchRequirement.alternativeMinimumOverallAverage
    )
  ) {
...
function formatCoreSubjects(route, language) {
  const subjects =
    language === "ar"
  ...
  if (!subjects.length) {
    return t(language).publishedCoreSubjects;
  }

  if (language === "ar") {
    return subjects
      .map((subject) => SUBJECT_LABELS_AR[subject] ?? subject)
      .join("، ");
  }
...
function evaluateAverage({
  ...
  const requiredMinimum = getPublishedMinimum(
    major,
    certificateSystem,
    detectedBranch
  );

  if (isForeignCertificateSystem(certificateSystem)) {
    return {
      status: "review",
      requiredMinimum: null,
      requirementText: text.foreignReviewRequirement,
      reason: text.foreignReviewReason,
    };
  }
  ...
  if (requiredMinimum === null) {
    return {
      status: "review",
      requiredMinimum: null,
      requirementText: text.missingMinimum,
      reason: text.missingMinimumReason,
    };
  }

  if (averageNumber >= requiredMinimum) {
    return {
      status: "eligible",
      requiredMinimum,
      requirementText:
        certificateSystem === "Tawjihi"
          ? text.tawjihiMinimum(requiredMinimum)
          : text.bagrutMinimum(requiredMinimum),
      reason: text.meetsMinimum(
        systemLabel(certificateSystem, language),
        requiredMinimum
      ),
    };
  }
  ...
  if (certificateSystem === "Tawjihi") {
    const alternative = getAlternativeTawjihiRoute(
      major,
      detectedBranch
    );
    ...
    if (
      alternative &&
      averageNumber >=
        alternative.alternativeMinimumOverallAverage
    ) {
      const requiredCoreAverage =
        alternative.alternativeMinimumCoreSubjectAverage;
      const subjects = formatCoreSubjects(
        alternative,
        language
      );

      if (coreSubjectAverageNumber === null) {
        return {
          status: "review",
          requiredMinimum,
          requirementText: text.standardAlternative(
            requiredMinimum,
            alternative.alternativeMinimumOverallAverage,
            requiredCoreAverage,
            subjects
          ),
          reason: text.missingCoreReason,
        };
      }

      if (coreSubjectAverageNumber >= requiredCoreAverage) {
        return {
          status: "eligible",
          requiredMinimum,
          usedAlternativeRoute: true,
          requirementText: text.alternativeRoute(
            alternative.alternativeMinimumOverallAverage,
            requiredCoreAverage,
            subjects
          ),
          reason: text.meetsAlternative,
        };
      }
      ...
      return {
        status: "notEligible",
        requiredMinimum,
        requirementText: text.alternativeCoreRequired(
          requiredCoreAverage,
          subjects
        ),
...
function evaluateMajor({
  ...
  if (!campusOk) {
    return {
      status: "notEligible",
      requiredMinimum: getPublishedMinimum(
        major,
        certificateSystem,
        detectedBranch
      ),
      reasons: [text.campusMismatch(campusPreference)],
      warnings: [],
    };
  }
  ...
  if (branchOk === false) {
    return {
      ...
      reasons: [text.branchMismatch],
      warnings: [],
    };
  }

  const averageEvaluation = evaluateAverage({
    major,
    certificateSystem,
    detectedBranch,
    averageNumber,
  ...
  const reasons = [averageEvaluation.reason];
  const warnings = [];

  if (branchOk === null) {
    warnings.push(text.branchReview);
  }

  if (
    isForeignCertificateSystem(certificateSystem) &&
    equivalencyConfirmed !== true
  ) {
    warnings.push(text.equivalencyWarning);
  }

  if (averageEvaluation.status === "notEligible") {
    return {
      ...averageEvaluation,
      status: "notEligible",
      reasons,
  ...
  if (
    averageEvaluation.status === "review" ||
    branchOk === null
  ) {
    return {
      ...averageEvaluation,
...
function calculateFitScore(
  ...
  for (const keyword of major.keywords ?? []) {
    ...
    if (
      ...
      score += 2;
      reasons.push(text.keywordMatch(keyword));
    }
  }
  ...
  const overlappingTerms = [
    ...new Set(
      searchText
        ...
        .filter((term) => programText.includes(term))
    ),
  ];

  if (overlappingTerms.length > 0) {
    score += Math.min(3, overlappingTerms.length);
    reasons.push(text.titleOverlap);
  }

  if (
    budgetNumber !== null &&
    Number.isFinite(major.tuitionPerHourNIS) &&
    major.tuitionPerHourNIS <= budgetNumber
  ) {
    score += 1;
    reasons.push(text.budgetFits(budgetNumber));
  }

  if (reasons.length === 0) {
    reasons.push(text.generalFit);
  }
...
function cleanSpecialNotes(major, language) {
  const notes = [
    ...
    return !GENERIC_NOTE_PARTS.some((part) =>
    ...
    ...new Set(
      notes.map(
        (note) =>
          ...
          "يرجى مراجعة عمادة القبول والتسجيل لتأكيد الشروط الإضافية الخاصة بهذا التخصص."
      )
    ),
  ];
}

function formatTuition(major, language) {
  const text = t(language);
  const scholarshipText = major.tuitionAfterScholarship
    ? ` ${text.afterScholarship}`
    : "";

  const currency = language === "ar" ? "شيكل" : "NIS";

  return `${major.tuitionPerHourNIS} ${currency}${scholarshipText}`;
}

function programName(major, language) {
  return language === "ar"
    ? `${major.programArabic} / ${major.programEnglish}`
    : `${major.programEnglish} / ${major.programArabic}`;
}

function collegeName(major, language) {
  return language === "ar"
    ? major.collegeArabic
    : major.collegeEnglish;
}
...
function formatMajor(
  ...
  const text = t(language);
  const tuition = formatTuition(major, language);

  const budgetNote =
    budgetNumber !== null &&
    major.tuitionPerHourNIS > budgetNumber
      ? `\n   ${text.budgetNote}: ${text.tuitionAboveBudget(
          tuition,
          budgetNumber
        )}.`
      : "";

  const tracks = major.programTracks?.length
    ? `\n   ${text.tracks}: ${major.programTracks.join(
        language === "ar" ? "، " : ", "
      )}`
    : "";

  const conditions = cleanSpecialNotes(major, language);
  ...
  const warningsText = major.eligibility.warnings.length
    ? `\n   ${text.reviewNotes}: ${major.eligibility.warnings.join(
        language === "ar" ? "؛ " : "; "
      )}`
    : "";

  return `${index}. ${programName(major, language)}
   ${text.status}: ${statusLabel}
   ${text.campusLabel}: ${campusLabel(
     major.campus,
     language
   )}
   ${text.college}: ${collegeName(major, language)}
   ${text.requirement}: ${major.eligibility.requirementText}
...
function formatCloseOption(major, index, language) {
  const text = t(language);
  ...
  return `${index}. ${programName(major, language)}
   ...
   ${text.requirementIssue}: ${major.eligibility.reasons.join(
     language === "ar" ? "؛ " : "; "
   )}`;
}

function explainInvalidSystem(value, language) {
  const text = t(language);
  const systems = Object.values(SYSTEM_LABELS[language]).map(
    (label) => `- ${label}`
  );

  return `${text.title}

${text.invalidSystemTitle}: "${value ?? ""}".
...
function explainInvalidBranch(value, language) {
  const text = t(language);
  const branches = TAWJIHI_BRANCHES.map(
    (branch) => `- ${branchLabel(branch, language)}`
  );

  return `${text.title}

${text.invalidBranchTitle}: "${value ?? ""}".
...
function explainInvalidAverage(
  ...
  const text = t(language);
  const explanation = outOfRange
    ? text.invalidAverageRange
    : text.averageHelp;

  return `${text.title}

${text.invalidAverageTitle}: "${average ?? ""}".
...
function resolveCertificateSystem(args) {
  const directSystem =
    detectCertificateSystem(args.certificateSystem) ||
    detectCertificateSystem(args.certificateType);

  if (directSystem) return directSystem;

  if (detectBranch(args.certificateType)) {
    return "Tawjihi";
  }
...
export function createReport(args = {}) {
  const language = detectReportLanguage(args);
  const text = t(language);
  const majors = loadMajors();
  const admissionSystems = loadAdmissionSystems();

  const certificateSystem = resolveCertificateSystem(args);

  if (!certificateSystem) {
    return explainInvalidSystem(
      args.certificateSystem ?? args.certificateType,
      language
    );
  }
  ...
  const detectedBranch = detectBranch(rawBranch);

  if (
    certificateSystem === "Tawjihi" &&
    !detectedBranch
  ) {
    return explainInvalidBranch(rawBranch, language);
  }

  const averageNumber = parseNumber(args.average);

  if (averageNumber === null) {
    return explainInvalidAverage(args.average, language);
  }
  ...
  const equivalencyConfirmed = parseBoolean(
  ...
  for (const major of majors) {
    const eligibility = evaluateMajor({
      major,
      certificateSystem,
      detectedBranch,
      averageNumber,
      coreSubjectAverageNumber,
      campusPreference: args.campusPreference,
      equivalencyConfirmed,
      language,
    });
  ...
  const sortMatches = (a, b) =>
    ...
    a.tuitionPerHourNIS - b.tuitionPerHourNIS ||
    a.programEnglish.localeCompare(b.programEnglish);
  ...
  const reviewSection = reviewRequired.length
    ? reviewRequired
        .slice(0, resultLimit)
        .map((major, index) =>
          formatMajor(
  ...
  const closeOptions = notEligible
    .filter((major) => {
      const required = major.eligibility.requiredMinimum;

      if (!Number.isFinite(required)) {
        return false;
      }

      const gap = required - averageNumber;
    ...
    .sort((a, b) => {
      const gapA =
        a.eligibility.requiredMinimum - averageNumber;
      const gapB =
        b.eligibility.requiredMinimum - averageNumber;

      return gapA - gapB || sortMatches(a, b);
    })
    .slice(0, 3);

  const closeOptionsSection = closeOptions.length
    ? closeOptions
        .map((major, index) =>
          formatCloseOption(major, index + 1, language)
        )
        .join("\n\n")
    : text.noClose;

  const averageLabel = isForeignCertificateSystem(
    certificateSystem
  )
  ...
  const preferredCampus =
    args.campusPreference &&
    normalizeText(args.campusPreference) !== "either"
      ? campusLabel(args.campusPreference, language)
      : CAMPUS_LABELS[language].Either;

  const foreignNotes = isForeignCertificateSystem(
    certificateSystem
  )
    ? [
        text.noteForeignOne,
...
- ${text.interests}: ${args.interests || text.notProvided}
- ${text.careerGoals}: ${
    args.careerGoals || text.notProvided
  }
- ${text.budget}: ${args.budget || text.notProvided}
